...
function findNumbers(string) {
    ...
    numberMap.set(
        nineOrSixOrZero[outerCount.findIndex((a) => a === 2)]
            .split("")
            .sort()
            .join(""),
        9
    ...
    numberMap.set(
        nineOrSixOrZero[outerCount.findIndex((a) => a === 1)]
            .split("")
            .sort()
            .join(""),
        6
    ...
    numberMap.set(
        nineOrSixOrZero[outerCount.findIndex((a) => a === 0)]
            .split("")
            .sort()
            .join(""),
        0
...
function removeAllLettersExceptOne(startingString, substringToRemove) {
    return startingString
        .split("")
        .filter((a) => !substringToRemove.includes(a))
        .join("");
}
...
function parseOutput2(digitMap, output) {
    const result = Number(
        output
            .split(" ")
            .map((a) => digitMap.get(a.split("").sort().join("")))
            .join("")
    );
