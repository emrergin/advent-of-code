...
const relatedMap = spaceMap.map(a=>a.split('').map(a=> a===`.` ? 0:1));

const xdimension = relatedMap.length;
const ydimension = relatedMap[0].length;
const scores = Array(xdimension).fill().map(a=>Array(ydimension).fill(0));
    ...
    for(let j=0;j<ydimension;j++){  
        ...
        for (let k=0;k<xdimension;k++){
            for(let l=0;l<ydimension;l++){
                ...
                if(relatedMap[k][l]===0 || ((i===k)&&(j===l))){continue;}
...
function solvePart2(coordX,coordY){
    let listOfAsteroids = [];
    let id=1;
    relatedMap.forEach((row,xindex) => row.forEach(
        (column,yindex)=> {
            if (column){
                listOfAsteroids.push({id:id++,
                                        y:yindex,
                                        x:xindex,
    ...
    function vaporize(kl){   
        let queue = listOfAsteroids.reduce((prev,curr)=>{
            let subarray = prev?.find(a=>a[0].angle===curr.angle);
            if (subarray){subarray.push(curr)}
            else{
                prev.push([curr])
            }
            return prev
        },[])
        .map(a=>a.filter((a)=>a.distance>0).sort((a,b)=>{
        ...
        .sort((a,b)=>
        {
            if(a[0].angle>b[0].angle){return -1}
            if(a[0].angle<b[0].angle){return 1}
            return 0
        })
