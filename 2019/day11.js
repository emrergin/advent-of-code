...
function operatorCaller(arrayToUse,inputMemory,outputMemory,relativeBaseM){
    ...
    while(currentIndex!==null && currentIndex<arrayToUse.length &&currentIndex!==`HALT`){        
        currentIndex = operator(arrayToUse,currentIndex,inputMemory,outputMemory,relativeBaseM);
        if (outputMemory.length==2){
            ...
            inputMemory=allTiles.filter(a=>a.x===currentLocation.x&&a.y===currentLocation.y).map(a=>a.color);
            if (inputMemory.length===0){inputMemory=[0]}
            outputMemory=[];           
        }        
    }

    function resolveLocation(location,color){
        let relevantTile=allTiles.filter(place=>place.x===location.x&&place.y===location.y)[0];
