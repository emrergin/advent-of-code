...
closestIntersectDistance = Infinity;

function pathGenerator(wire){
	let pathPoint={x:0,y:0,length:0}
	path=[];
	for(i=0;i<wire.length;i++){
		let currentCommandDirection=wire[i].charAt(0);
		let currentCommandLength=wire[i].slice(1);
		
		while(currentCommandLength){
			switch (currentCommandDirection){
				case `U`:
					pathPoint.y+=1;					
				break;
				case `R`:
			...
			pathPoint.length+=1;
			path.push({x:pathPoint.x,y:pathPoint.y,length:pathPoint.length});
...
const path1=pathGenerator(wire1).sort((a,b)=>a.length-b.length);
const path2=pathGenerator(wire2).sort((a,b)=>a.length-b.length);

loop:for(i=1;i<path1.length;i++){
	for(j=1;j<path2.length;j++){
		if (path1[i].x===path2[j].x && path1[i].y===path2[j].y){
			console.log(path1[i].length+path2[j].length);
			break loop;
		}
	}
}
