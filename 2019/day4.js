function countCombinations(start,end){
	...
	outerloop: for(i=start;i<=end;i++){
		...
		for(k=0;k<arrayFromNumber.length;k++){
			if(arrayFromNumber[k]===arrayFromNumber[k+1] && arrayFromNumber[k-1]!==arrayFromNumber[k] && arrayFromNumber[k]!==arrayFromNumber[k+2]){
				doubleController=true;
			}			
		}
		if (!doubleController){continue outerloop}
