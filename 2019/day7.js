...
function solvePart2(){
	...
	for (let permutation of allPermutations){
		...
		while(true){
			...
			if(computers[activeOperator].currentIndex==='HALT'){break;}
			if (computers[activeOperator].output.length===1){
				computers[(activeOperator+1)%5].memory.push(computers[activeOperator].output.pop())
	...
	function Computer(intcode,directive){
		...
		this.run = () => {
			this.currentIndex = operator(this.intcode,this.currentIndex,this.memory,this.output);
		};
	}
}
