...
for(let i=0;i<commands.length;i++){
    ...
    if(command.slice(0,4)==='$ cd'){
        if(command.slice(-1)==='/'){
            ...
            currentDirectory=new Directory(currentDirectory.path
                ...
                +command.substring(4),currentDirectory);
        }
    }
    if(command.slice(0,4)==='$ ls'){
        ...
        temporaryMemory=temporaryMemory.map(a=>a.split(' ')).filter(a=>a[0]!=='dir').map(c=>[+c[0],c[1]])
        const totalAddedSize = temporaryMemory.reduce((acc,curr)=>acc+curr[0],0);
