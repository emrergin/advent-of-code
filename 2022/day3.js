...
function findCommonType(com1,com2,com3){
    ...
    if (arguments[2]===undefined){
        ...
        filteredArray = [...com1].filter(value =>  [...com2].includes(value)&&[...com3].includes(value));
    }
...
function partTwo(){
    ...
    for(let i=0;i<lines.length;i=i+3){
        const currentThreeElves = [].concat(lines[i]).concat(lines[i+1]).concat(lines[i+2]);
        totalPriorityOfBadges+=convertToPriority(findCommonType(currentThreeElves[0],currentThreeElves[1],currentThreeElves[2]));
    }
