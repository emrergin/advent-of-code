...
function controlNeighbourToAdd(xx,yy,currentLetter){
    ...
    if(convertToPriority(targetLetter)-convertToPriority(currentLetter)>1){   
        return false;
    }
    return true;
}
...
function controlNeighbourToAdd2(xx,yy,currentLetter){
    ...
    if(convertToPriority(currentLetter)-convertToPriority(targetLetter)>1){        
        return false;
    }
    return true;
}
...
function convertToPriority(letter){
    if(letter==="E"){return "{".charCodeAt(0);}
...
function findDistanceFrom(tarX,tarY,controlFunction,part){
    ...
    for (let y=0;y<commands.length;y++){
        for (let x=0;x<commands[0].length;x++){
            ...
            controlFunction(x+1,y,commands[y][x]) && tile.neighbours.push(`x:${x+1}-y:${y}`);
            controlFunction(x-1,y,commands[y][x])&& tile.neighbours.push(`x:${x-1}-y:${y}`);
            controlFunction(x,y+1,commands[y][x]) && tile.neighbours.push(`x:${x}-y:${y+1}`);
            controlFunction(x,y-1,commands[y][x]) && tile.neighbours.push(`x:${x}-y:${y-1}`);
...
function partOne(){
    ...
    const result = findDistanceFrom(startingPoint.x,startingPoint.y,controlNeighbourToAdd,1);
    console.log(result);
}
...
function partTwo(){
    ...
    const result = findDistanceFrom(startingPoint.x,startingPoint.y,controlNeighbourToAdd2,2);
    console.log(result);   
}
