...
let commands = readFileSync(`day15input.txt`, 'utf-8').split("\n")
...
function partTwo(){
    ...
    let totalXtotalY = commands.reduce((acc,curr)=>({x:acc.x+curr.sensor.x*(totalDistance/curr.distance),
    y:acc.y+curr.sensor.y*(totalDistance/curr.distance),z:acc.z+(totalDistance/curr.distance)}),{x:0,y:0,z:0});

    totalXtotalY={x:Math.round(totalXtotalY.x/totalXtotalY.z),y:Math.round(totalXtotalY.y/totalXtotalY.z)}
    ...
    outerloop: for(let i=0;i<totalDistanceToMove;i++){
        ...
        let currentPoint = ({x: totalXtotalY.x+i*Math.sign(totalXtotalY.x-xToMoveAwayFrom),
        y: totalXtotalY.y+(totalDistanceToMove-i)*Math.sign(totalXtotalY.y-yToMoveAwayFrom)})
        for(let command of commands){
