...
function partTwo(){
    ...
    const  n = maxY-minY;
    const  l = maxX-minX;
    let arr = Array(m+1).fill().map(() => Array(n+1).fill().map(() => Array(l+1)));
            ...
            for (let j=minY;j<=n;j++){
                for (let k=minX;k<=l;k++){
                    if(arr[i-minZ][j-minY][k-minX]===0 &&
                        ((i===minZ || j===minY || k==minX || i===maxZ || j===maxY || k===maxX )
                        ||(i-minZ>=1 && arr[i-minZ-1][j-minY][k-minX]==='.')||(i-minZ+1<= m && arr[i-minZ+1][j-minY][k-minX]==='.')
                        ||(j-minY>=1 && arr[i-minZ][j-minY-1][k-minX]==='.')|| (j-minY+1<=n && arr[i-minZ][j-minY+1][k-minX]==='.')
                        ||(k-minX>=1 && arr[i-minZ][j-minY][k-minX-1]==='.') || (k-minX+1<=l && arr[i-minZ][j-minY][k-minX+1]==='.'))){
                        arr[i-minZ][j-minY][k-minX]='.';
                    }
                }
            }
        }
        if(JSON.stringify(tempCopy)===JSON.stringify(arr)){break;}
        counter++;
    ...
    for (let i=0;i<commandWithAdditions.length;i++){
        for (let j=i+1;j<commandWithAdditions.length;j++){
            const temp = distanceBetweenCubes(commandWithAdditions[i],commandWithAdditions[j]);
            if(temp<=1){
                total-=2;
            }
        }
