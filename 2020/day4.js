...
let lines = readFileSync(`day4input.txt`, 'utf-8')
                .split('\n\n').map(a=>a.split(" ").flatMap(c=>c.split("\n")).map(b=>b.split(":")))
...
outerloop: for(let line of lines){
    ...
    if(!(numControl(line.get("byr"),1920,2002)&&numControl(line.get("iyr"),2010,2020)&&numControl(line.get("eyr"),2020,2030)
    &&heightCheck(line.get("hgt"))&&hairControl(line.get("hcl"))&&eyeControl(line.get("ecl"))&&pidControl(line.get("pid")))){
        continue;
    }
    counter++;
}
